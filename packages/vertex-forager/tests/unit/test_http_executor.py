...
class TestHttpExecutorAuthentication:
    ...
    @pytest.mark.asyncio
    async def test_bearer_auth(self, mock_client: AsyncMock) -> None:
        ...
        spec = RequestSpec(url="https://api.example.com/data", auth=auth)

        await executor.fetch(spec)

        call_args = mock_client.request.call_args
        assert call_args[1]["headers"]["Authorization"] == "Bearer test_token_123"

    @pytest.mark.asyncio
    async def test_header_auth(self, mock_client: AsyncMock) -> None:
        ...
        call_args = mock_client.request.call_args
        assert call_args[1]["headers"]["X-API-Key"] == "api_key_xyz"

    @pytest.mark.asyncio
    async def test_query_param_auth(self, mock_client: AsyncMock) -> None:
        """Test query parameter authentication."""
        from vertex_forager.core.config import RequestAuth

        executor = HttpExecutor(client=mock_client)
        auth = RequestAuth(kind="query", token="secret_key", query_param="api_key")
        spec = RequestSpec(url="https://api.example.com/data", auth=auth)
    ...
    @pytest.mark.asyncio
    async def test_no_auth(self, mock_client: AsyncMock) -> None:
...
class TestDefaultAsyncClient:
    """Tests for default async client factory."""

    def test_creates_client_with_correct_user_agent(self) -> None:
        """Test default client has correct User-Agent."""
        from vertex_forager.core.http import default_async_client

        client = default_async_client()
        assert client.headers["User-Agent"] == "vertex-forager"

    def test_creates_client_with_correct_timeout(self) -> None:
        """Test default client has correct timeout."""
        from vertex_forager.core.http import default_async_client

        client = default_async_client()
        assert client.timeout.read == 60.0

    def test_creates_client_instance(self) -> None:
        """Test default client is created correctly."""
        from vertex_forager.core.http import default_async_client
        import httpx

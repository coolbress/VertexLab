...
from vertex_forager.core.controller import GradientConcurrencyLimiter, GCRARateLimiter


@pytest.mark.asyncio
async def test_gradient_window_rollover_resets_min_rtt() -> None:
    limiter = GradientConcurrencyLimiter(initial_limit=3, min_limit=1, max_limit=5, queue_size=1, smoothing=0.2)
    ...
    assert math.isclose(limiter.min_rtt, 0.5, rel_tol=1e-6) and limiter.min_rtt != old_min

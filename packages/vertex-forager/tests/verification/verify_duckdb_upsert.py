...
async def main():
    ...
    async with DuckDBWriter(db_path) as writer:
        ...
        df2 = pl.DataFrame({
            ...
            "ticker": ["AAPL"],
            "date": [date(2024, 1, 1)],
            "open": [100.0], "high": [105.0], "low": [99.0], "close": [150.0],
            "volume": [1000], "closeadj": [150.0], "closeunadj": [150.0],
            "lastupdated": [date(2024, 1, 1)],
            "fetched_at": [datetime.now()]
        })
    ...
    print("\n--- 4. Verification ---")
    conn = duckdb.connect(str(db_path))
    result = conn.execute("SELECT close FROM sharadar_sep WHERE ticker='AAPL' AND date='2024-01-01'").fetchone()
    conn.close()

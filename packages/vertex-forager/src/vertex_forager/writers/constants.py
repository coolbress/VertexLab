...
LOG_NULLS_PK_COL: Final[str] = (
    "{prefix}: Nulls detected in PK column '{col}' for table '{table}'. rows={rows}, columns={columns}"
)
...
LOG_SKIP_UNIQUE_INDEX_MISSING_PK: Final[str] = (
    "{prefix}: Skipping UNIQUE INDEX for {table}: Missing PK columns {missing} in dataframe"
)
...
LOG_CORRELATION_SUMMARY: Final[str] = (
    "{prefix}: Correlation summary: trace_ids={trace_ids_count}, request_ids={request_ids_count}"
)
